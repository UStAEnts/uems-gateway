...
export class EventGatewayAttachment implements GatewayAttachmentInterface {
    async generateInterfaces(send: GatewayMk2.SendRequestFunction): Promise<GatewayInterfaceActionType[]> {
        ...
        return [
            ...
            },
            {
                action: 'get',
                path: '/events',
                handle: EventGatewayAttachment.getEventsHandler(send),
                additionalValidator: validator,
            },
            {
                action: 'patch',
        ...
        return async (req: Request, res: Response) => {
            ...
            const callback: RequestCallback = (
                ...
                if (status === EventRes.MsgStatus.SUCCESS) {
                    ...
                    httpRes.status(HttpStatus.NOT_FOUND).json(ErrorCodes.FAILED);
                }
            };
        ...
        return async (req: Request, res: Response) => {
            ...
            const callback: RequestCallback = (
                ...
                if (status === EventRes.MsgStatus.SUCCESS) {
                    ...
                    httpRes.status(HttpStatus.NOT_FOUND).send(ErrorCodes.FAILED);
                }
            };
    ...
    private static getEventsHandler(send: SendRequestFunction) {
        return async (req: Request, res: Response) => {
            ...
            const callback: RequestCallback = (
                ...
                if (status === EventRes.MsgStatus.SUCCESS) {
                    ...
                    httpRes.status(HttpStatus.SERVICE_UNAVAILABLE).send(ErrorCodes.FAILED);
                }
            };
